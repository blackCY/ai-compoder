...
interface HomeStageOutputDisplayProps {
  output: StageOutput;
}

export const HomeStageOutputDisplay: React.FC<HomeStageOutputDisplayProps> = ({ output }) => {
  return (
    <div className="space-y-3">
      ...
      <div>
        {output.selectedComponents?.length && (
          <div className="text-blue-400 font-semibold text-xs mb-2 flex items-center gap-1.5">
            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
            ...
            Selected Components
            <span className="text-gray-500 text-[10px] bg-gray-700/50 px-1.5 py-0.5 rounded">
              {output.selectedComponents?.length}
            </span>
          </div>
        )}
